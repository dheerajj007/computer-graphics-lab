...
int main()
{
    ...
    int rotation[][3] = { { cos(rad), -sin(rad), 0 }, { sin(rad), cos(rad), 0 }, { 0, 0, 1 } };

    // Define the scaling matrix
    int scaling[][3] = { { 2, 0, 0 }, { 0, 2, 0 }, { 0, 0, 1 } };
    ...
    for (i = 0; i < 3; i++) {
        for (j = 0; j < 3; j++) {
            new_triangle[i][j] = 0;
            for (k = 0; k < 3; k++) {
                new_triangle[i][j] += triangle[i][k] * rotation[k][j];
            }
            new_triangle[i][j] *= scaling[j][j];
        }
    }
